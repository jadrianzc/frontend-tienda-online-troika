...
const Pago = ({ dataInfo, menuState, setAddCar, addCar }) => {
    ...
    const LoadData = async (idUserSession) => {
      try {
        ...
        const dataCarUser = res.data.filter((data) =>
          data.idUserSession === idUserSession ? data : null
        );
        setRows(dataCarUser);
      } catch (error) {
        console.log(error);
  ...
  const postData = async (idUserSession, data) => {
    const resp = await axios.post(
      `http://localhost:4000/api/v1/usuarios/${idUserSession}/orden-compra`,
      data
    );
          ...
          <Grid item className="grid-item-metodoPago">
            <FormGroup row className="grid-item-metodoPago-form">
              ...
              <p className="grid-item-metodoPago-form-p">
                Realice el pago de su pedido a nuestra cuenta en cualquier
                agencia de nuestro banco asociado o realice una transferencia
                bancaria por el monto a pagar. Los datos de su orden de pago y
                nuestro número de cuenta llegaran a su correo electrónico.
              </p>
            </FormGroup>
          </Grid>
