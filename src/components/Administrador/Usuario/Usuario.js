...
const Usuario = () => {
	...
	const RegistrarUsuario = async (datos) => {
		try {
			await axios.post('https://server-tienda-troika.herokuapp.com/api/v1/usuarios', datos);
			setStado(!stado);
			setOpenAlert(true);
		} catch (error) {
			console.log(error);
			...
			<Grid container justifyContent="space-between" className="grid-container-add-user">
				<Grid item className="grid-item-user-4">
					<div className="grid-item-title">
						<Typography className="title-usuario title">Agregar usuario</Typography>
					</div>
				...
				<Grid item className="grid-item-user-6">
					<div className="grid-item-title">
						<Typography className="title-usuario title">Búsqueda de usuario</Typography>
					</div>
