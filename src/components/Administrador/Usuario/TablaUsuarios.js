...
function TablaUsuarios(stado) {
	...
	useEffect(() => {
		const LoadData = async () => {
			try {
				const res = await axios.get(`https://server-tienda-troika.herokuapp.com/api/v1/usuarios`);
	...
	const EliminaUsuario = async () => {
		try {
			await axios.delete(`https://server-tienda-troika.herokuapp.com/api/v1/usuarios/${iduser}`);
			setEtado(!estado);
			setOpenModal(false);
			setOpenAlert(true);
		} catch (error) {
	...
	const AcrualizaUser = async () => {
		try {
			await axios.put(`https://server-tienda-troika.herokuapp.com/api/v1/usuarios/${contUser._id}`, contUser);
			setEtado(!estado);
	...
	const bodyElimi = (
		<div className="ModalRegistro">
			...
			<input type="button" className="btnPwd btnPwdAcep" value="Continuar" onClick={EliminaUsuario} />
			<input
				type="button"
	...
	const bodyEdit = (
		<div className="ModalRegistro">
			<Grid item className="grid-item-user-data">
				<Grid container className="grid-container-user-data user-edit">
					...
					<Grid item className="grid-item-info">
						<label>Nombres</label>
						<input type="text" name="nomb_usuario" value={contUser.nomb_usuario} required onChange={handlechangeUser} />
					</Grid>
					<Grid item className="grid-item-info">
					...
					<Grid item className="grid-item-info">
						...
						<input type="text" name="ced_usuario" value={contUser.ced_usuario} required onChange={handlechangeUser} />
					</Grid>
					<Grid item className="grid-item-info">
						<label>E-mail*</label>
						<input
			...
			<input type="button" className="btnPwd btnPwdAcep" value="Actualizar" onClick={AcrualizaUser} />
			<input
				type="button"
				className=" btnPwd btnPwdCan"
				value="Cancelar"
								...
								<TableRow key={doc._id}>
									...
									<TableCell>
										<div className="contAccioAdmin">
											<button className="btnEditAdmin" onClick={() => IdEdit(doc)}>
												Edit
											</button>
