...
import { Container, Grid, Typography, Modal, Snackbar } from '@material-ui/core';
...
function AdminProductos() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			await axios.post('https://api.cloudinary.com/v1_1/troikafoto/image/upload', dataimg).then((res) => {
			...
			<form method="post" onSubmit={handleSubmit}>
				<Grid container className="grid-container-user-data user-edit">
					<Grid container item xs={3}>
						<input accept="image/*" name="foto" id="contained-button-file" type="file" required />
					</Grid>
				...
				<Grid item container className="grid-item-produc-4" xs={12}>
					<Grid item className="grip-agrega-produc">
						<div className="grid-item-title">
							<Typography className="title-usuario title">Agregar producto</Typography>
						</div>
						<Grid item className="grid-item-user-data">
							<Grid container className="grid-container-user-btn" alignItems="center">
					...
					<Grid item className="grip-busca-produc">
						<div className="grid-item-title">
							<Typography className="title-usuario title">Búsqueda de producto</Typography>
						</div>
						<Grid container justifyContent="space-between" alignItems="center">
							<form onSubmit={handleSubmitBusqueda} style={{ display: 'contents' }}>
								<label>Código:</label>
